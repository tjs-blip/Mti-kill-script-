--[[ 
    ...
    Fully descendant-safe version with robust NPC + Player attack
--]]
...
----------------------------------------------------
-- 3. TARGETING FUNCTIONS (DESCENDANT SAFE)
----------------------------------------------------
...
local function getPlayerCharacterModels()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if char then
                for _, hum in ipairs(char:GetDescendants()) do
...
padding.PaddingTop = MAIN_PADDING
padding.PaddingBottom = MAIN_PADDING
...
----------------------------------------------------
-- 7. CORE ATTACK LOOP (DESCENDANTS SAFE)
----------------------------------------------------
RunService.Heartbeat:Connect(function()
    local now = os.clock()

    if not (isNPCToggled or isPlayerKillToggled) then return end
    if now - lastAttackTime < currentInterval then return end
    lastAttackTime = now

    local combinedTargets = {}

    local function addValidTargetsFromModels(models)
        for _, model in ipairs(models) do
            for _, hum in ipairs(model:GetDescendants()) do
                if hum:IsA("Humanoid") and hum.Health > 0 then
                    table.insert(combinedTargets, hum)
                end
            end
        end
    end
    ...
    if isNPCToggled then
        addValidTargetsFromModels(getNonPlayerCharacterModels())
    end
    if isPlayerKillToggled then
        addValidTargetsFromModels(getPlayerCharacterModels())
    end
