--[[ 
    ...
    MODIFICATION: The getNonPlayerCharacterModels function has been made more robust 
    by aggressively iterating through all descendants of the Workspace to find Humanoids 
    and their containing models, ensuring we capture NPCs deep within containers.
--]]
...
----------------------------------------------------
-- 3. TARGETING FUNCTIONS (ROBUST GetDescendants IMPLEMENTATION)
----------------------------------------------------
-- Helper function to find a Humanoid using GetDescendants.
...
local function getNonPlayerCharacterModels()
    ...
    local processedModels = {} -- Used to ensure we only add a character model once

    -- Iterate through ALL descendants of the Workspace
    for _, descendant in ipairs(Workspace:GetDescendants()) do
        -- Only care about Humanoid instances
        if descendant:IsA("Humanoid") then
            local model = descendant.Parent
            
            -- The Humanoid's parent MUST be the character model (or a model wrapper)
            if model and model:IsA("Model") and model ~= Character and not processedModels[model] then
                
                local player = Players:GetPlayerFromCharacter(model)
                
                -- It is an NPC if no player is associated with the character model
                if not player then
                    table.insert(nonPlayerModels, model)
                    processedModels[model] = true -- Mark model as processed
                end
            end
        end
    end
...
local function getPlayerCharacterModels()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            -- Use the deep-search function (which uses GetDescendants) to confirm the character model is valid
            if char and findHumanoidInModel(char) then 
    ...
    if (isNPCToggled or isPlayerKillToggled) and (now - lastAttackTime >= currentInterval) then
        ...
        local function addValidTargets(targets)
            for _, model in ipairs(targets) do
                -- This check ensures the model has a non-dead Humanoid, found via deep-search
