--[[ 
    ...
    CRITICAL FIX: The ControlFrame now explicitly handles input (InputBegan) across its 
    entire surface, making the non-button areas "tangible" and draggable.
--]]
...
GUIToggleButton.AutoButtonColor = false -- Critical: Ensure input is registered
GUIToggleButton.ZIndex = 2 -- Ensure visibility and clickability
...
InputRow.ZIndex = 2
InputRow.Active = true -- Make this row tangible too, just in case
...
-- Robust Drag Function: listens for input on the frame itself
local function setupFrameDrag(frame)
    ...
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- CRITICAL FIX: Drag only if the input target IS the frame.
            -- Because we set frame.Active=true, the click won't pass through to
            -- the background, making it register as the frame.
