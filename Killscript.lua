--[[ 
    ...
    DRAG FUNCTION V4: The dragging functionality is now applied to a dedicated 
    "Drag Handle" (header bar) and the GUIToggleButton for maximum reliability.
--]]
...
local FRAME_HEIGHT = 240 -- Increased to fit new handle
...
-- We need a separate frame to hold the main content so it doesn't overlap the handle
local ContentFrame = Instance.new("Frame", ControlFrame)
ContentFrame.Name = "ContentFrame"
ContentFrame.Size = UDim2.new(1, 0, 1, -HANDLE_HEIGHT) -- 100% width, height minus the handle
...
-- 4.3. NPC Toggle Button (Moved to ContentFrame)
...
-- 4.4. Player Kill Button (Moved to ContentFrame)
...
-- 4.5. Interval Input Group (Moved to ContentFrame)
...
----------------------------------------------------
-- 6. DRAG AND CLICK EVENT HANDLERS (REWORKED V4)
----------------------------------------------------

-- Reworked Drag Function (V4): Applies drag logic to the element provided.
-- This works because DragHandle and GUIToggleButton are the only elements that will call it.
local function setupDragElement(elementToDrag, elementToListenOn)
    ...
    elementToListenOn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if input.Target == elementToListenOn then -- Ensure we only drag the intended element
                isDragging = true
                dragStart = input.Position
                initialPosition = elementToDrag.Position
                elementToDrag.ZIndex = 99 -- Bring to top during drag
            end
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            elementToDrag.Position = UDim2.new(initialPosition.X.Scale, initialPosition.X.Offset + delta.X, 
    ...
    UserInputService.InputEnded:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
            isDragging = false
            -- Reset ZIndex
            elementToDrag.ZIndex = (elementToDrag == ControlFrame) and 1 or 2 
        end
    end)
end
...
-- Apply the drag logic:
-- 1. Drag the ControlFrame using the DragHandle (TextLabel)
setupDragElement(ControlFrame, DragHandle) 
-- 2. Drag the GUIToggleButton using itself
setupDragElement(GUIToggleButton, GUIToggleButton)
