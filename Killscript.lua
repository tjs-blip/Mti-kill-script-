--[[ 
    ...
    CRITICAL FIX: Implemented a robust dragging method using a transparent overlay 
    inside the ControlFrame to ensure dragging works even when clicking near 
    child elements (buttons/textboxes).
--]]
...
-- DRAG OVERLAY: This is the invisible surface that will handle dragging
local DragOverlay = Instance.new("ImageLabel")
DragOverlay.Name = "DragOverlay"
DragOverlay.Size = UDim2.fromScale(1, 1)
DragOverlay.BackgroundTransparency = 1 -- Make it invisible
DragOverlay.ImageTransparency = 1 
DragOverlay.ZIndex = 0 -- Keep it behind the interactive elements
DragOverlay.Parent = ControlFrame 

-- Ensure all interactive elements have a ZIndex higher than 0
...
NPCToggleButton.ZIndex = 1 -- Ensure higher ZIndex
...
PlayerKillButton.ZIndex = 1 -- Ensure higher ZIndex
...
InputGroup.ZIndex = 1 -- Ensure higher ZIndex
...
IntervalLabel.ZIndex = 1 -- Ensure higher ZIndex
...
InputRow.ZIndex = 1 -- Ensure higher ZIndex
...
IntervalTextBox.ZIndex = 1 -- Ensure higher ZIndex
...
SetButton.ZIndex = 1 -- Ensure higher ZIndex
...
----------------------------------------------------
-- 6. DRAG AND CLICK EVENT HANDLERS (REWORKED & FIXED)
----------------------------------------------------

-- Robust Drag Function: takes the element *to be dragged* and the element 
-- *to listen for input* (which is the frame itself, or the drag overlay)
local function setupFrameDrag(frameToMove, inputHandler)
    local isDragging = false
    local dragStart = Vector2.new(0, 0)
    local startPos = UDim2.new(0, 0, 0, 0)

    local dragBegin = inputHandler.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            
            -- IMPORTANT: If the input target is the inputHandler itself, start drag
            if input.Target == inputHandler then 
                isDragging = true
                dragStart = input.Position
                startPos = frameToMove.Position
                frameToMove.ZIndex = 2
            end
        end
    end)
    
    -- Tracks mouse/touch movement while dragging
    local dragMove = UserInputService.InputChanged:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            
            frameToMove.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, 
                                       startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)

    -- Stops dragging when the mouse/touch is released anywhere
    local dragEnd = UserInputService.InputEnded:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
            isDragging = false
            frameToMove.ZIndex = 1
        end
    end)
end
...
-- Apply the robust drag logic:
-- 1. Main Control Frame (listens on the DragOverlay, moves the ControlFrame)
setupFrameDrag(ControlFrame, DragOverlay)

-- 2. GUI Toggle Button (listens on and moves the GUIToggleButton itself)
setupFrameDrag(GUIToggleButton, GUIToggleButton)
