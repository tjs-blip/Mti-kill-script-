--[[ 
    ...
    FIX: The drag logic has been completely reworked to ensure the GUIToggleButton 
         and ControlFrame are reliably draggable without interfering with child buttons.
    
    GUI UPGRADES: The interface is now larger, clearer, and better spaced.
--]]
...
----------------------------------------------------
-- 6. DRAG AND CLICK EVENT HANDLERS (REWORKED)
----------------------------------------------------

-- Robust Drag Function
local function setupFrameDrag(frame)
    ...
    -- Detects when the user presses down on the frame (or its background)
    dragBegin = frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            
            -- Only start dragging if the input target IS the frame itself,
            -- or if it's the GUIToggleButton (which is a TextButton but serves as the main frame for dragging).
    ...
    -- Stops dragging when the mouse/touch is released
    dragEnd = frame.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            isDragging = false
            frame.ZIndex = 1
        end
    ...
    -- IMPORTANT: For the ControlFrame, we must also stop dragging on the UserInputService level
    -- to catch releases outside the frame area.
