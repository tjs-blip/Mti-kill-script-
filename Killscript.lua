--[[ 
    ...
    DRAG FUNCTION V3: Reworked the dragging logic to be globally robust, ensuring 
    the frame is draggable by clicking anywhere on its background area.
--]]
...
ControlFrame.Active = true -- CRITICAL: Makes the frame's background surface clickable/tangible
...
InputRow.ZIndex = 2
InputRow.Active = true -- Also set active
...
----------------------------------------------------
-- 6. DRAG AND CLICK EVENT HANDLERS (REWORKED V3)
----------------------------------------------------

-- Reworked Drag Function (V3): More reliable drag using global InputChanged
local function setupFrameDrag(frame)
    local isDragging = false
    local dragStart = nil
    local initialPosition = nil

    -- 1. Start drag when input begins on the frame (or the button, which is the frame in this context)
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            
            -- This check is kept to prevent dragging if they click a child button/textbox 
            -- (since those also fire InputBegan on the parent, but the target is the child)
            if input.Target == frame then
                isDragging = true
                dragStart = input.Position
                initialPosition = frame.Position
                frame.ZIndex = 99 -- Bring to top during drag
            end
        end
    end)
    
    -- 2. Move the frame using global InputChanged
    UserInputService.InputChanged:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            
            frame.Position = UDim2.new(initialPosition.X.Scale, initialPosition.X.Offset + delta.X, 
                                       initialPosition.Y.Scale, initialPosition.Y.Offset + delta.Y)
            
            -- Update initial position for continuous movement calculation
            initialPosition = frame.Position 
            dragStart = input.Position
        end
    end)

    -- 3. Stop drag globally on InputEnded
    UserInputService.InputEnded:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
            isDragging = false
            -- Reset ZIndex based on if it's the main frame or the toggle button
            frame.ZIndex = (frame == ControlFrame) and 1 or 2 
        end
    end)
end
...
-- Apply the drag logic to both frames
